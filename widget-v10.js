(function() {
    ...
    // 1. Inject the CSS (Now with Mobile Responsiveness!)
    const styles = `
        #my-mock-widget { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 10px; background: #fff; width: 100%; max-width: 400px; margin: 0 auto; box-sizing: border-box; overflow-x: hidden; }
        #my-mock-widget * { box-sizing: border-box; }
        #my-mock-widget .demo-header { text-align: center; margin-bottom: 15px; }
        #my-mock-widget .demo-logo { max-width: 150px; height: auto; margin-bottom: 10px; }
        #my-mock-widget .demo-text { font-size: 13px; color: #666; font-style: italic; line-height: 1.4; margin: 0; padding: 0 10px; }
        
        #my-mock-widget .calendar-scroll-area { max-height: 40vh; overflow-y: auto; overflow-x: hidden; padding-right: 5px; margin-bottom: 15px; }
        #my-mock-widget .calendar-scroll-area::-webkit-scrollbar { width: 6px; }
        #my-mock-widget .calendar-scroll-area::-webkit-scrollbar-thumb { background: #ccc; border-radius: 4px; }

        #my-mock-widget .calendar-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 6px; text-align: center; }
        #my-mock-widget .day-name { font-weight: bold; font-size: 0.85em; color: #777; padding-bottom: 10px; }
        #my-mock-widget .calendar-day { padding: 10px 5px; border: 1px solid #e0e0e0; border-radius: 6px; cursor: pointer; transition: all 0.2s ease; display: flex; flex-direction: column; align-items: center; justify-content: center; background: #fafafa; }
        ...
        #my-mock-widget .date-num { font-size: 1em; font-weight: bold; }
        #my-mock-widget .price { font-size: 0.75em; color: #28a745; margin-top: 4px; font-weight: 600; }
        
        #my-mock-widget .modal-footer { text-align: center; border-top: 1px solid #eee; padding-top: 15px; }
        #my-mock-widget #selectedInfo { margin-bottom: 15px; font-size: 0.95em; color: #555; min-height: 20px;}
        
        #my-mock-widget .book-btn { display: inline-block; padding: 14px 24px; background-color: #3C0C3D; color: white; text-decoration: none; border-radius: 6px; font-size: 16px; font-weight: bold; width: 100%; transition: background 0.3s; }
        ...
        @media (max-width: 480px) {
            ...
            #my-mock-widget .calendar-grid { gap: 3px; }
            #my-mock-widget .day-name { font-size: 0.75em; padding-bottom: 5px; }
            #my-mock-widget .calendar-day { padding: 6px 2px; }
            #my-mock-widget .date-num { font-size: 0.85em; }
            #my-mock-widget .price { font-size: 0.65em; margin-top: 2px; }
            #my-mock-widget .calendar-scroll-area { max-height: 55vh; } /* Gives more vertical room on phones */
            #my-mock-widget .book-btn { padding: 12px 20px; font-size: 15px; }
        }
    `;

(function() {
    ...
    // Helper Function: Aggressively hunt for the CMS's native close button and click it
    function closeCMSModal() {
        const elements = document.querySelectorAll('button, a, div, span, i');
        let clicked = false;
        
        for (let el of elements) {
            // Skip our own Book Now button
            if (el.id === 'bookBtn') continue; 
            
            const aria = (el.getAttribute('aria-label') || '').toLowerCase();
            // Ensure classname is parsed safely
            const cls = (el.className && typeof el.className === 'string') ? el.className.toLowerCase() : '';
            const txt = el.textContent.trim().toLowerCase();
            
            // Look for strong indicators of a close button
        ...
        // Final fallback: Look for any button inside the modal wrapper that isn't our Book Now button
        ...
        // Note: We no longer hide the targetDiv here. If we miss the button, we leave the calendar visible so you don't get trapped in a white box.
    }
